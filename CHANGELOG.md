...
- **個股頁空狀態改為熱門標的圖卡**：原本只有一個 📈 圖示與「輸入股票代號」提示，現在直接呈現三個排行榜（成交值／漲幅／跌幅，各 8 檔，可切換），每張卡有代碼、名稱、現價、漲跌幅、成交值與近一個月 sparkline，點擊即查詢該股。
  - 新增 `services/popular_service.py` 與 `GET /api/stock/popular?limit=`（上限 20，預設 8）。**路由必須註冊在 `/{ticker}` 之前**，否則會被那條 catch-all 當成股票代碼吃掉。
  - **排行零額外外部 API 呼叫**：完全由 `stock_service` 既有的全市場報價快照（TWSE + TPEx Open API，30 分鐘快取）計算。為此在 `_TWQuote` 補上 `turnover` 欄位，取交易所原始的成交值（TWSE `TradeValue` / TPEx `TransactionAmount`），而非收盤價×股數的近似值——顯示「1036.2 億」這種數字要禁得起查核。
  - sparkline 以 `yf.download` 一次批次抓三榜去重後的 24 檔（約 1 秒；逐檔 `Ticker().history()` 要數秒），結果連同排行快取 30 分鐘。整包 JSON 約 8.8 KB。
  - 前端 sparkline 是手刻 SVG polyline 而非 recharts：使用者在這個畫面還沒查任何個股，為 8 條迷你線載入約 340 kB 的圖表庫並不划算。
- **走勢圖期間擴充到 5年 / 10年 / 全部**（原本最長 1 年）。台股 `max` 回溯至 2000-01（yfinance 資料起點，非個股真實上市日）；美股為完整歷史。

### Fixed（2026-07-29 熱門標的與長期歷史）

- **進入個股頁後無法回到熱門標的頁**：新增三條返回路徑——搜尋框下方的「‹ 熱門標的」鍵、搜尋框的清除鈕（此頁初始狀態即卡片頁，只清文字會停在個股頁上）、以及修好的瀏覽器上一頁。後者原本壞在兩處：
  - `navigate()` 一律帶 `replace: true`，從卡片頁進個股時把卡片頁那筆歷史蓋掉了，上一頁會直接離開 Stock 頁。改為從卡片頁進個股用 push、個股之間切換用 replace（避免連查五檔要按五次上一頁）。
  - `symbol` 只在首次 render 由 `useState(paramSymbol ?? "")` 初始化，之後網址再變都不同步回 state；即使修好歷史紀錄，按上一頁也只有網址列會動、畫面不變。新增 effect 讓網址成為查詢對象的單一真實來源，並在其中集中處理個股資料清理（否則按上一頁時 `priceData` 殘留，會與熱門標的同時出現在畫面上）。
- **熱門標的排行混入 ETF**：首版過濾條件只寫「4 位純數字代碼」，但台股 4 碼 ETF（0050、0056）同樣符合，實測 0050 以 457 億成交值排進成交值榜第二名。改為排除開頭為 0 的代碼（普通股代碼範圍 1101–9958），權證與 5–6 碼 ETF 則本來就被長度條件擋下。

### Changed（2026-07-29 熱門標的與長期歷史）

- **長期間走勢圖自動降頻**：`_PERIOD_SPEC` 新增 K 棒週期欄位，10年改用週線、全部改用月線，各期間點數壓在 1,500 以內。`max` 若照日線傳輸，台股是 6,606 根、美股逾 11,000 根，JSON 逼近 400 kB 且 recharts 的 hover 會明顯卡頓。前端在圖表標題標示實際週期（「收盤走勢（月線）」），避免月線被誤讀成日線；`TechnicalResponse` 為此新增 `history_interval`。
- **指標計算改為固定 1 年日線**（`_INDICATOR_PERIOD` / `_INDICATOR_INTERVAL`），與圖表期間徹底脫鉤。降頻之後這點從「較好」變成「必要」：若沿用圖表的抓取結果，切到「全部」會讓 MA5 變成月線 MA5（即 5 個月均線），語意完全走樣。圖表序列與指標參數相同時沿用同一次抓取，不同時才多打一次。實測 2330.TW 在 1mo/3mo/6mo/1y/5y/10y/max 七個期間下，MA60 恆為 2346.03、RSI 恆為 35.2。
- **漲跌幅榜設 3,000 萬元成交值門檻**：沒有門檻的話榜單會被冷門股洗版，它們一兩筆委託就能拉出 9% 的漲幅，對使用者沒有參考價值。成交值榜本身即流動性排序，不另設限。
